...
import { releasePackageAcrossBranchMoves, type BranchTracker } from './branch-integration';
...
export async function runReleaseLoop<TResult>(
  ...
  for (const name of order) {
    ...
    const { released, version, result } = dryRun
      ? await releaseOne(pkg, bumpsForThisPackage)
      : await releasePackageAcrossBranchMoves(name, branchTracker(workspace, log), async () => await releaseOne(pkg, bumpsForThisPackage), log);
...
/** The release branch as `releasePackageAcrossBranchMoves` needs to see it, backed by the real repository. */
function branchTracker(workspace: Workspace, log: (message: string) => void): BranchTracker {
  return {
    integrate: async () => await integrateBranchTip(workspace, log),
    tip: async () => await fetchBranchTip(await currentBranch({ cwd: workspace.root }), { cwd: workspace.root }),
  };
}
